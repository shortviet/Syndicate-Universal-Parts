...
function ScriptsTab.Init(ctx)
    ...
    local function tw(obj, t, props, sty, dir)
        ...
        local info = TweenInfo.new(t or 0.2, sty or Enum.EasingStyle.Quart, dir or Enum.EasingDirection.Out)
    ...
    local function twP(obj, t, props, sty, dir)
        if typeof(obj) ~= "Instance" then return { Play = function() end, Cancel = function() end } end
        local info = TweenInfo.new(t or 0.15, sty or Enum.EasingStyle.Quart, dir or Enum.EasingDirection.Out)
        local twn = TweenService:Create(obj, info, props)
        twn:Play()
        return twn
    end
    local function twC(slot, obj, t, props, sty, dir)
        if slot and slot.tween then pcall(function() slot.tween:Cancel() end) end
        local tween = tw(obj, t, props, sty, dir)
        if slot then slot.tween = tween end
        return tween
    end

    -- Player helpers
    local flyActive = ctx.flyActive or false
    local sendNotif = ctx.sendNotif or function(title, text, dur) print("[SU] " .. tostring(title) .. ": " .. tostring(text)) end
    local getNearestPlayer = ctx.getNearestPlayer or function()
    ...
    local function cleanRow(parent, yPos, label, badge, badgeCol, initOn, onToggle)
        local ROW_H = 42
        local row = Instance.new("Frame", parent)
        row.Size = UDim2.new(1, 0, 0, ROW_H); row.Position = UDim2.new(0, 0, 0, yPos)
        row.BackgroundColor3 = C.bg2 or Color3.fromRGB(26, 26, 28); row.BackgroundTransparency = 0
        row.BorderSizePixel = 0; corner(row, 12)

        local rowDot = Instance.new("Frame", row)
        rowDot.Size = UDim2.new(0, 3, 0, 20); rowDot.Visible = false; rowDot.Position = UDim2.new(0, 0, 0, 11)
        rowDot.BackgroundColor3 = badgeCol or C.accent; rowDot.BackgroundTransparency = 0.4
        rowDot.BorderSizePixel = 0; corner(rowDot, 99)

        local lbl = Instance.new("TextLabel", row)
        lbl.Size = UDim2.new(0, 160, 1, 0); lbl.Position = UDim2.new(0, 16, 0, 0)
        lbl.BackgroundTransparency = 1; lbl.Text = label or ""
        lbl.Font = Enum.Font.GothamBold; lbl.TextSize = 13
        lbl.TextColor3 = C.text; lbl.TextXAlignment = Enum.TextXAlignment.Left

        local badgeLbl = Instance.new("TextLabel", row)
        badgeLbl.Size = UDim2.new(0, 140, 1, 0); badgeLbl.Position = UDim2.new(0, 170, 0, 0)
        badgeLbl.BackgroundTransparency = 1; badgeLbl.Text = badge or ""
        badgeLbl.Font = Enum.Font.GothamBold; badgeLbl.TextSize = 9
        badgeLbl.TextColor3 = C.sub; badgeLbl.TextXAlignment = Enum.TextXAlignment.Left

        local togTrack = Instance.new("Frame", row)
        togTrack.Size = UDim2.new(0, 32, 0, 18); togTrack.Position = UDim2.new(1, -44, 0.5, -9)
        togTrack.BackgroundColor3 = C.bg3 or Color3.fromRGB(34, 34, 38)
        togTrack.BackgroundTransparency = 0.2; togTrack.BorderSizePixel = 0; corner(togTrack, 99)

        local cStr = _makeDummyStroke(togTrack)
        if type(cStr) == "table" and cStr.Thickness ~= nil then
            cStr.Thickness = 1; cStr.Color = C.accent; cStr.Transparency = 0.6
        end

        local togKnob = Instance.new("Frame", togTrack)
        togKnob.Size = UDim2.new(0, 12, 0, 12)
        togKnob.Position = UDim2.new(0, 2, 0.5, -6)
        togKnob.BackgroundColor3 = Color3.fromRGB(100, 100, 110)
        togKnob.BackgroundTransparency = 0; togKnob.BorderSizePixel = 0; corner(togKnob, 99)

        local _C3_WHITE = Color3.new(1, 1, 1)
        local _C3_SUB2 = Color3.fromRGB(100, 100, 110)
        local togState = initOn or false
        local activeCol = badgeCol or C.accent
        ...
        local function setToggle(on, animate)
            ...
            if on then
                ...
                if type(cStr) == "table" and cStr.Color ~= nil then
                    pcall(function() twP(cStr, 0.15, { Color = activeCol, Transparency = 0.5 }) end)
                end
                rowDot.Visible = true
            else
                twP(togTrack, 0.15, { BackgroundColor3 = C.bg3 or Color3.fromRGB(34, 34, 38), BackgroundTransparency = 0.2 })
                twP(togKnob, 0.15, { BackgroundColor3 = _C3_SUB2, Position = UDim2.new(0, 2, 0.5, -6) })
                if type(cStr) == "table" and cStr.Color ~= nil then
                    pcall(function() twP(cStr, 0.15, { Color = C.accent, Transparency = 0.6 }) end)
                end
                rowDot.Visible = false
            end
        end

        local function getToggle() return togState end
        local function setVisualFn(on) togState = on; setToggle(on, false) end
        ...
        rowBtn.MouseEnter:Connect(function()
            twP(row, 0.08, { BackgroundColor3 = C.bg3 or Color3.fromRGB(34, 34, 38) })
        end)
        rowBtn.MouseLeave:Connect(function()
            twP(row, 0.08, { BackgroundColor3 = C.bg2 or Color3.fromRGB(26, 26, 28) })
        end)
        rowBtn.MouseButton1Click:Connect(function()
            togState = not togState
            setToggle(togState)
            if onToggle then onToggle(togState) end
        end)

        if togState then setToggle(togState) end
        return row, setToggle, getToggle, setVisualFn
    end

    local function _AF_loadAndPlayAnimation(hum, animId)
    ...
    local _flyMuteSounds = ctx._flyMuteSounds or function() end
